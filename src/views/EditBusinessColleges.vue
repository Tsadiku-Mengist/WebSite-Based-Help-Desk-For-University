...
<!--main content-->
 <div class="flex pl-80 bg-gray-200" >
       <div class="mt-3 pl-40 ">
          ...
          <!--acount image-->
       <div class="pl-5">
             <img :src="businessCollege.photo" alt="" class="w-[200px] rounded-full mt-10">
         </div>
...
import Spinner from '../components/Spinner.vue';
    ...
    methods: {
        updateSubmit: async function () {
            try {
                let response = await ContactService.updateBusinessCollege(this.businessCollege, this.businessCollegeId);
                if (response) {
                    return this.$router.push("/ContactBusinessColleges");
                }
                else {
                    return this.$router.push(`/ContactBusinessColleges/edit/${this.businessCollegeId}`);
                }
            }
            catch (error) {
                console.log(error);
